...
describe('EWP-P3B-TS06 — registered adapter lifecycle authority', () => {
  describe('family 1 — static lifecycle authority', () => {
    ...
    test('EWP-P3B-TS06 red: generic kernels have no known-tool, global, or root-ordinal policy', () => {
      ...
      for (const path of lifecycleSources) {
        ...
        if (knownToolPolicy.test(text))
          issues.push(`${path} contains executable known-tool policy`);
  ...
  describe('family 2 — registry-bound selection and exits', () => {
  ...
  describe('family 3 — fixture immutable lifecycle planning', () => {
  ...
  describe('family 4 — fixture placement and plan-to-binding identity', () => {
    ...
    test('EWP-P3B-TS06 red: fixture scoped routing reaches the private execution binding', async () => {
      ...
      if (rootFacts === null || listScoped === null || resolveScoped === null) {
        issues.push(
          'registered writable fixture has no complete normalized scoped placement shape',
        );
      } else {
  ...
  describe('family 5 — fixture verification and gate policy', () => {
    ...
    test('EWP-P3B-TS06 red: one pure gate reducer owns strict verdict semantics', async () => {
      ...
      if (
        ...
        issues.push(
          'acquire/run.ts and place/run.ts still duplicate strict gate verdict reduction',
        );
      }
      if (
        gateModule !== null &&
  ...
  describe('family 6 — adapter-provided renderer facts', () => {
    ...
    test('EWP-P3B-TS06 red: pure renderers consume already-selected adapter facts', async () => {
      ...
      const verify = await renderSelected(
        ...
        (tool: string) =>
          tool === FIXTURE_P3B_WRITE_TOOL ? FIXTURE_P3B_DEEP_COVERAGE_SUFFIX : null,
  ...
  describe('family 7 — Claude and Codex lifecycle parity', () => {
  ...
  describe('family 8 — read-only tools stay effect-free', () => {
    test('EWP-P3B-TS06 characterization: every read-only application path is effect-free exit 4', async () => {
      ...
      for (const tool of ['kilo-code', 'opencode']) {
        ...
        for (const deep of [false, true]) {
          ...
          expect(outcome.exitClass, `verify/${tool}/${deep ? 'deep' : 'static'}`).toBe(
            'capability',
          );
  ...
  describe('family 9 — relevant capability fingerprints', () => {
