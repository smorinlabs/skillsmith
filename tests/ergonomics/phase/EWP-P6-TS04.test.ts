import { describe, expect, test } from 'bun:test';
import { readFile } from 'node:fs/promises';
import { resolve } from 'node:path';
import { buildProgram } from '../../../packages/cli/src/program.ts';
import { CURRENT_COMMAND_SPECS } from '../../../packages/cli/src/spec/index.ts';

const ROOT = resolve(import.meta.dir, '..', '..', '..');

const commandSection = (reference: string, name: string): string => {
  const marker = `### \`${name}\``;
  const start = reference.indexOf(marker);
  expect(start, marker).toBeGreaterThanOrEqual(0);
  const tail = reference.slice(start);
  const boundaries = [tail.indexOf('\n### ', marker.length), tail.indexOf('\n## ', marker.length)]
    .filter((index) => index >= 0)
    .toSorted((left, right) => left - right);
  return tail.slice(0, boundaries[0] ?? tail.length);
};

describe('EWP-P6-TS04', () => {
  test('generated README/reference output closes against the live command tree', async () => {
    const generatorPath = '../../../scripts/generate-command-reference.ts';
    const [reference, readme, generator] = await Promise.all([
      readFile(resolve(ROOT, 'docs/commands.md'), 'utf8'),
      readFile(resolve(ROOT, 'README.md'), 'utf8'),
      import(generatorPath) as Promise<Record<string, unknown>>,
    ]);
    expect(typeof generator.renderCommandReference).toBe('function');
    expect(typeof generator.checkCommandReference).toBe('function');
    expect(typeof generator.validateReadmeCommandIndex).toBe('function');
    expect(typeof generator.renderReadmeCapabilityMatrix).toBe('function');
    expect(typeof generator.validateReadmeCapabilityMatrix).toBe('function');
    const validateMarkers = generator.validateReadmeCommandIndex as (value: string) => string[];
    const validateCapabilityMarkers = generator.validateReadmeCapabilityMatrix as (
      value: string,
    ) => string[];
    const startMarker = '<!-- skillsmith-command-index:start -->';
    const endMarker = '<!-- skillsmith-command-index:end -->';
    const capabilityStart = '<!-- skillsmith-capability-matrix:start -->';
    const capabilityEnd = '<!-- skillsmith-capability-matrix:end -->';
    expect(validateMarkers(`${startMarker}\n${endMarker}`)).toEqual([]);
    expect(validateMarkers(`${startMarker}\n${startMarker}\n${endMarker}`)).toHaveLength(1);
    expect(validateMarkers(`${endMarker}\n${startMarker}`)).toHaveLength(1);
    expect(validateCapabilityMarkers(`${capabilityStart}\n${capabilityEnd}`)).toEqual([]);
    expect(
      validateCapabilityMarkers(`${capabilityStart}\n${capabilityStart}\n${capabilityEnd}`),
    ).toHaveLength(1);
    expect(validateCapabilityMarkers(`${capabilityEnd}\n${capabilityStart}`)).toHaveLength(1);
    expect(reference).toStartWith('<!-- Generated by scripts/generate-command-reference.ts. -->');
    expect(readme).toContain(startMarker);
    expect(readme).toContain(endMarker);
    expect(readme.split(startMarker)).toHaveLength(2);
    expect(readme.split(endMarker)).toHaveLength(2);
    expect(readme.split(capabilityStart)).toHaveLength(2);
    expect(readme.split(capabilityEnd)).toHaveLength(2);
    for (const command of buildProgram().commands) {
      expect(reference, command.name()).toContain(`\`${command.name()}\``);
      for (const alias of command.aliases()) expect(reference, alias).toContain(`\`${alias}\``);
    }
    for (const spec of CURRENT_COMMAND_SPECS.filter(
      (candidate) => candidate.path.split(' ').length === 2,
    )) {
      const section = commandSection(reference, spec.path.slice('skillsmith '.length));
      expect(section, spec.path).toContain(`Primary question: ${spec.primaryQuestion}`);
      expect(section, spec.path).toContain(`Minimal invocation: \`${spec.minimalInvocations[0]}\``);
      for (const alias of spec.aliases)
        expect(section, `${spec.path} alias ${alias}`).toContain(alias);
      for (const workflow of spec.commonWorkflows) {
        expect(section, `${spec.path} workflow ${workflow.label}`).toContain(
          `**${workflow.label}** (${workflow.safety}): ${workflow.description} — \`${workflow.invocation}\``,
        );
      }
      expect(
        spec.commonWorkflows.map((workflow) => workflow.invocation),
        spec.path,
      ).toEqual(spec.examples);
      for (const argument of spec.arguments) {
        const term = `${argument.required ? '<' : '['}${argument.name}${argument.variadic ? '...' : ''}${argument.required ? '>' : ']'}`;
        expect(section, `${spec.path} ${argument.name}`).toContain(
          `\`${term}\` — ${argument.description}`,
        );
      }
      for (const option of spec.options) {
        expect(section, `${spec.path} ${option.long}`).toContain(`\`${option.flags}\``);
      }
      for (const exit of spec.exitCodes ?? []) {
        expect(section, `${spec.path} exit ${exit.code}`).toContain(
          `\`${exit.code}\` — ${exit.meaning}`,
        );
      }
    }
  });
});
