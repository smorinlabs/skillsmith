...
const execJsonRpcCommand = async (
  command: string,
  ...
  try {
    ...
    throw toPortError(error, { capability: 'process', operation: 'exec', context: { command } });
  }
};
...
const createProcessPorts = (): {
  ...
  const exec: ProcessPort['exec'] = async (command, args, options) => {
    if (options?.jsonRpc) {
      ...
      for (const name of options.unsetEnv ?? []) delete childEnvironment[name];
      const result = await execJsonRpcCommand(command, args, options, childEnvironment);
      if (result.protocolError === 'cancelled') {
        throw toPortError(
          { code: 'ABORT_ERR' },
          {
            capability: 'process',
