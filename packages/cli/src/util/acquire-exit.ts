...
export const acquireExitCode = (
  report: Readonly<{
    results: readonly Readonly<{ error?: SkillSmithError }>[];
  }>,
): ExitCode => selectExitCode(report.results.map((r) => (r.error ? exitCodeForError(r.error) : 0)));
