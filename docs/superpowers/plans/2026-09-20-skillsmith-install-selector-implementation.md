...
Use existing lifecycle error envelopes and exit classes: invalid usage, source failure, permission failure, and cancellation remain distinguishable. Permission errors identified by the Git adapter exit 6; some Git subprocess permission failures remain source errors and exit 5 because Git does not return structured permission information. Either classification stops the incomplete scan. No-match and ambiguity use the existing per-source report paths. Messages identify the requested name and matching tier safely; never include raw frontmatter, parser excerpts, or unbounded Git output.
